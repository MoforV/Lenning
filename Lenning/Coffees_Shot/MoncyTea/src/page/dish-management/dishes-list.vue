<template>
	<div class="ordering" v-loading.fullscreen.lock="loading">
		<div class="heading">饮品管理</div>
		<div class="content-view">
			<div class="button-view">
				<router-link :to="{name:'upload'}">
				<el-button type="success" size="medium">添加饮品</el-button>
				</router-link>
			</div>
			<div v-if="nodatas">
				<!-- 表头 -->
			...
			<!-- 没有数据 -->
			<div class="nodatas" v-if="!nodatas">还没有饮品数据</div>
		</div>
...
export default{
	data() {
		return {
			...
			tablist: ['创建时间','类目','饮品名称','饮品图片','价格(元)','操作'],
			tabcont:[],
			currentnum:1,//当前页数
			total:0,//总条数
			pagenum:0,
